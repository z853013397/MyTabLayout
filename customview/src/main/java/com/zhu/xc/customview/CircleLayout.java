...
public class CircleLayout extends ViewGroup {
	public interface OnItemClickListener {
		void onItemClickListener(CircleLayout cl, int tag, int position);
	}
	...
	// private int horizontalSpacing = 20;// 没一行直接的间距
	private int[] count;// 总共的个数
	private static final String TAG = "CircleLayout";

	private static final int item_height = 72;//dp
	
	/** 用来标记是哪个layout 方便在onitemclck中调用*/
	private int tag;

	/**
	 * @param context
	 * @param tag 声明是哪个tag 方便调用
	 */
	public CircleLayout(Context context, int tag) {
		this(context, null);
		this.tag = tag;
	}
	...
	private void initView() {
		distance = dp2Px(item_height);
		CircleView view = null;
		if (count.length == -1)
			return;
			...
			view.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if (mOnItemClickListener != null) {
						mOnItemClickListener.onItemClickListener(CircleLayout.this, tag, position);
					}
				}
			});
			addView(view);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int item = cCount / number;// 这个表示有多少行

		if (cCount % number != 0) {// 如果有余数 就加1
			item++;
		}
		int height = distance * item;
	...
	// CustomView view = (CustomView) getChildAt(position);
	// view.changeToCircleColor(color);
	// invalidate();
	// }

	@Override
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		int itemWidth = getMeasuredWidth() / 5; // 每个item的宽度
		int count = getChildCount();
		for (int i = 0; i < count; i++) {
			View childView = getChildAt(i);
			int childWidth = childView.getMeasuredWidth();// 子view 的width的宽度
			int childHeight = childView.getMeasuredHeight();// distance
			int empthWidth = (itemWidth - childWidth) / 2;//
			// int distanceLeft = (i % number) * (childWidth + empthWidth);//
			// onlayout
			int distanceLeft = (i % number) * itemWidth + empthWidth; // left
			// int top = (horizontalSpacing + childHeight) * (i / number);
			int top = distance * (i / number) + (distance - childHeight) / 2;
			Log.i(TAG, "top的艰巨：" + top);
			childView.layout(distanceLeft, top, distanceLeft + childWidth, top + childHeight);
		}
	 ...
	 * @param position
	 */
	public void changeRightState(int position) {
		CircleView view = (CircleView) getChildAt(position);
		// view.changeRightState();
	}
	 ...
	 * @param position
	 */
	public void changeErrorState(int position) {
		CircleView view = (CircleView) getChildAt(position);
		// view.changeErrorState();
	}
	 ...
	 * @param position
	 */
	public void changeHalfState(int position) {
		CircleView view = (CircleView) getChildAt(position);
		// view.changeHalfState();
	}
